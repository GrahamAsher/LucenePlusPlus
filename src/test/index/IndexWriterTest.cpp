    ...
    virtual void eval(const MockRAMDirectoryPtr& dir) {
        if (doFail) {
            if (TestPoint::getTestPoint(L"abort") || TestPoint::getTestPoint(L"flushDocument")) {
                ...
                throw (IOException(L"now failing on purpose"));
            }
        }
    }
};
    ...
    virtual void eval(const MockRAMDirectoryPtr& dir) {
        if (doFail) {
            if (TestPoint::getTestPoint(L"MockRAMDirectory", L"sync")) {
                didFail = true;
                throw (IOException(L"now failing on purpose during sync"));
            }
        }
    }
};
...
public:
    virtual void eval(const MockRAMDirectoryPtr& dir) {
        ...
        if (isCommit) {
            if (!isDelete) {
                fail1 = true;
                throw (RuntimeException(L"now fail first"));
            } else {
                fail2 = true;
                throw (IOException(L"now fail during delete"));
            }
        }
    }
};
...
public:
    virtual bool testPoint(const String& name) {
        if (doFail && name == L"DocumentsWriter.ThreadState.init start") {
            throw (RuntimeException(L"intentionally failing"));
        }
        return true;
    }
};
...
public:
    virtual void eval(const MockRAMDirectoryPtr& dir) {
        ...
        if (TestPoint::getTestPoint(L"SegmentMerger", L"mergeTerms") && !didFail1) {
            didFail1 = true;
            throw (IOException(L"fake disk full during mergeTerms"));
        }
        if (TestPoint::getTestPoint(L"BitVector", L"write") && !didFail2) {
            didFail2 = true;
            throw (IOException(L"fake disk full while writing BitVector"));
        }
    }
};
